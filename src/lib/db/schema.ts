...
export const users = pgTable("users", {
  ...
  // setup yet (see src/app/api/auth/activate/route.ts). A non-null value
  ...
  // confirmedAt null — a QR was issued via /aktivasi but never
  // successfully scanned+verified), or "confirmed" (both set).
  totpConfirmedAt: timestamp("totp_confirmed_at"),
  // One-time invite/activation code, generated by scripts/seed-user.ts (or
  // scripts/reissue-activation.ts) and given to the user out-of-band
  // (WA/lisan). Required alongside the username on /aktivasi before a TOTP
  // QR is ever issued — this is what stops anyone who merely knows/guesses
  // a registered username from claiming its TOTP setup. Cleared (set to
  // null) the moment it's successfully used, so it can't be reused or
  // brute-forced indefinitely. Null also correctly describes "no pending
  // activation" for already-confirmed users.
  activationCode: varchar("activation_code", { length: 12 }),
  activationCodeExpiresAt: timestamp("activation_code_expires_at"),
